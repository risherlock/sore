...
function [w, zn_d, zxd] = feedforward_control(phy_params, state, xdd_d, xd_d, x_d, K, dt, zn_d, zxd)
  % Physical params
  g = phy_params(1);
  m = phy_params(2);
  l = phy_params(3);
...
for t = 1:length(time)-1
  ...
  [w(:,t), zn_d, zxd] = feedforward_control(phy_params, state(:,t), xdd_d, xd_d, x_d, K, dt, zn_d, zxd);
...
w(:,end) = feedforward_control(phy_params, state(:,t), xdd_d, xd_d, x_d, K, dt, zn_d, zxd);

figure;
plot(time, jounce, 'LineWidth', 1);
title("Jounce");
grid on; grid minor;

figure;
plot(time, jerk, 'LineWidth', 1);
title("Jerk");
grid on; grid minor;

figure;
plot(time, acceleration, 'LineWidth', 1);
title("Acceleration");
grid on; grid minor;

figure;
plot(time, velocity, 'LineWidth', 1);
title("Velocity");
grid on; grid minor;

figure;
plot(time, position, 'LineWidth', 1);
title("Position");
grid on; grid minor;

% Position
figure;
plot(time, state(1,:), 'LineStyle', '-', 'LineWidth', 1); hold on;
plot(time, state(2,:), 'LineStyle', '--', 'LineWidth', 1);
plot(time, state(3,:), 'LineStyle', ':', 'LineWidth', 1);
legend('x', 'y', 'z');
...
% Orientation
figure;
plot(time, rad2deg(state(7,:)), 'LineStyle', '-', 'LineWidth', 1); hold on;
plot(time, rad2deg(state(8,:)), 'LineStyle', '--', 'LineWidth', 1);
plot(time, rad2deg(state(9,:)), 'LineStyle', ':', 'LineWidth', 1);
legend('\phi', '\theta', '\psi');
...
% Control input
figure;
plot(time, w(1,:), 'LineStyle', '-', 'LineWidth', 1); hold on;
plot(time, w(2,:), 'LineStyle', '--', 'LineWidth', 1);
plot(time, w(3,:), 'LineStyle', '-.', 'LineWidth', 1);
plot(time, w(4,:), 'LineStyle', ':', 'LineWidth', 1);
xlabel('Time (s)');
